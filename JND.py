...
foil = visual.GratingStim(
    win, sf=1, size=4, mask="gauss", ori=expInfo["refOrientation"]
)
target = visual.GratingStim(
    win, sf=1, size=4, mask="gauss", ori=expInfo["refOrientation"]
)
fixation = visual.GratingStim(
    win, color=-1, colorSpace="rgb", tex=None, mask="circle", size=0.2
)
...
message2 = visual.TextStim(
    win,
    pos=[0, -3],
    text="Then press left or right to identify the %.1f deg probe."
...
feedback1 = visual.TextStim(
    win, pos=[0, +3], text="mean of final 6 reversals = %.3f" % (approxThreshold)
)
