...
# 目前模拟的就是1素材的dong，fox，none实验
# 先使用试验结果获得对应的noise_level顺序，不用snr数据，后续会重新计算
# changeplace：需要改存储实验结果的文件路径
dirname={1:"fox",2:"cat",3:"dog",4:"deer",5:"horse"}
file_path =os.path.join(os.path.dirname(__file__), "get_multchoice_result/1/snr_values.txt")
...
snr_choice_list,snrnone_list=getlongaudio(factor_path, noise_levels,choices,truechoice,laststr="_0.wav",fps=0.5)
...
# 听觉提供的drift文件保存在"dog_fox_drift"
# changeplace：改存储路径,保存的文件名表示正确选项，利于后续使用
save_path=os.path.join(os.path.dirname(__file__), "dog_fox_drift")
