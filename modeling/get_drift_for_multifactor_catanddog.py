...
# 存储视觉权重的地方，即实验结果路径
# changeplace：
main_path=os.path.join(os.path.dirname(__file__), "第二部分实验/1")
...
# # 存储听觉的地方,由cal_snr_multfactor得到的snr的保存路径
# changeplace
factor_dir=os.path.join(os.path.dirname(__file__), "cat_dog_drift")
# 文件的形式是：
"""
'dog': [1, 2, 3, 4],
'fox': [5, 6, 7, 8],
...
# 保存得到的最终的drift的路径
# changeplace
save_path = os.path.join(os.path.dirname(__file__), "cat_dog_drift_timelen")
...
for i in os.listdir(factor_dir):
    ...
    att_path=os.path.join(main_path,"weight_matrix.txt")
...
# changeplace:根据是视+音/视/音来进行判别，其中视+音/视这里都是加法，
# 音则是减法，因为此时视频是错误选项的信息增益方，由于是2选择所以在这直接减就行不用加给错误选项
# 三个情况使用的音视频不同，所以在这要分开运行
